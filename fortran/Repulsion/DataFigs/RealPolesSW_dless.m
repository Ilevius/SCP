function RealPolesSW_dless(fname,key,f1,f2,nf)

% Real dispersion curves for PlateWax by lines -- dimensionless

%   fname -  file with the pole data computed in RealPoles
 % key = 0 - wavenumbers; 1 - slowness    
% [fd1,fd2] - frequency range for the axis of abscissa - dimensionless
%  nf - starting number of figure windows to be opened

% Call example: RealPolesSW_dless('fs_SW_3.dat',1,0,0.65,1)
...
while n > 0
 ...
 %   xlabel('frequency f [kHz]')
  ...
  % dimensionless axese
    xlabel('frequency f ')
   if key == 0  ylabel('wavenumbers  \zeta_n ');   end
   if key == 1  ylabel('slownesses   s_n ');   end
