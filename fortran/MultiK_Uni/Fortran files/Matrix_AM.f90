...
! Forms Ck+, Ck-
! Mk - matrix of eigenvectors 
! Tk - matrix of stress
! Gk - matrix 4*4 (U(1:2, 1:4), Tk(1:2, 1:4)) 
...
! Tk, Gk, Mk, etc.
      ...
      if(k == 1)then      
        AM(1:2,1:4)=Ckp(3:4,1:4)
        
        if(Ns > 1)then        
          AM(3:6,1:4)=Ckm(1:4,1:4)
        else
          go to 1  ! k=Ns        
        end if ! Ns=1
...
! acoust bottom Ck+
          ...
          Tk=0d0; Tk(1,1)=-w*w*rho(Ns); 
          Gk(3:4,1:4)=Tk(1:2,1:4)
          Ckp=MATMUL(Gk,Mkp); 
          Ckp(1,1:4)=0d0; Ckp(1,2)=1d0
...
! k=Ns         
 ...
 1  k=Ns
    ...
    select case(iK)
      ...
      case(2)
        SMN(1:2,1:4)=Ckm(3:4,1:4)
      case(3)
        SMN(1:2,1:4)=Ckm(1:2,1:4)
      case(4)
        SMN = 0d0; SMN(1,3)=1d0; SMN(2,4)=1d0;        
    end select ! iK
